...
use Hesper\Core\OSQL\OrderChain;
use Hesper\Core\OSQL\QueryIdentification;
...
final class Criteria extends QueryIdentification {
	 ...
	 * @return Criteria
	 **/
	public function fetchCollection($path, // to collection
	...
	public function get() {
		try {
			$list = [$this->checkAndGetDao()->getByQuery($this->toSelectQuery())];
		...
		foreach ($proto->getPropertyList() as $property) {
			if (($property instanceof LightMetaProperty) && $property->getRelationId() == MetaRelation::ONE_TO_ONE && !$property->isGenericType() && ((!$property->getFetchStrategyId() && ($this->getFetchStrategy()->getId() == FetchStrategy::JOIN)) || ($property->getFetchStrategyId() == FetchStrategy::JOIN))) {
				...
				if (!$query->hasJoinedTable($tableAlias)) {
					$logic = Expression::eq(DBField::create($property->getColumnName(), $parentTable),

						DBField::create($propertyDao->getIdName(), $tableAlias));
	...
	private function getProto() {
		return call_user_func([$this->checkAndGetDao()->getObjectName(), 'proto']);
	}
}
