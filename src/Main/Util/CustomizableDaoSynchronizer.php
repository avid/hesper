...
class CustomizableDaoSynchronizer {
	...
	private function convertMasterObjectToSlave($masterObject) {
		if ($masterObject && (call_user_func([$this->slave->getObjectName(), 'create']) instanceof SynchronizableObject)) {
			$masterObject = call_user_func([$this->slave->getObjectName(), 'createFromMasterObject'], $masterObject);
		}
