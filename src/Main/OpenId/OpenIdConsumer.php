...
use Hesper\Main\Crypto\CryptoFunctions;
use Hesper\Main\Crypto\DiffieHellmanKeyPair;
use Hesper\Main\Crypto\DiffieHellmanParameters;
use Hesper\Main\Flow\HttpRequest;
use Hesper\Main\Flow\Model;
use Hesper\Main\Flow\ModelAndView;
use Hesper\Main\Math\BigNumberFactory;
use Hesper\Main\Math\RandomSource;
use Hesper\Main\Net\Http\HttpClient;
...
final class OpenIdConsumer {
	 ...
	 * @param null        $manager
	 *
	 * @return OpenIdConsumerFail|OpenIdConsumerPositive
	 * @throws OpenIdException
	...
	public function doContinue(HttpRequest $request, $manager = null) {
		...
		if ($manager && ($association = $manager->findByHandle($parameters['openid.assoc_handle'], self::ASSOCIATION_TYPE)) && !isset($parameters['openid.invalidate_handle'])) { // smart mode
			$tokenContents = null;
		...
		} elseif (!$manager || isset($parameters['openid.invalidate_handle'])) { // dumb or handle invalidation mode
