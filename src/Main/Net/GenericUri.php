...
class GenericUri implements Stringable {
	...
	public function __toString() {
		return (is_null($this->scheme)||is_null($this->host))?$this->toStringFromRoot():$this->toString();
	}
	...
	public function normalize() {
		...
		// 2. percent-encoded
		$this->setHost($this->normalizePercentEncoded($this->getHost(), $this->hostNameCharPattern(false)))->setUserInfo($this->normalizePercentEncoded($this->getUserInfo(), $this->userInfoCharPattern(false)))->setPath(self::removeDotSegments($this->normalizePercentEncoded($this->getPath(), '\/' . $this->segmentCharPattern(false))))->setQuery($this->normalizePercentEncoded($this->getQuery(), $this->fragmentOrQueryCharPattern(false)))->setFragment($this->normalizePercentEncoded($this->getFragment(), $this->fragmentOrQueryCharPattern(false)));
	...
	private function normalizePercentEncoded($string, $unreservedPartChars) {
		...
		$result = preg_replace_callback('/((' . self::PATTERN_PCTENCODED . ')|(.))/sui', [PercentEncodingNormalizator::create()->setUnreservedPartChars($unreservedPartChars), 'normalize'], $string);
