...
use Hesper\Main\EntityProto\Builder\DTOToFormImporter;
...
abstract class PrototypedSoapClient {
	...
	public function __construct() {
		...
		$this->soapClient = new \SoapClient($wsdlUrl, ['soap_version' => SOAP_1_1, 'classmap' => $this->classMap(),
	...
	protected function call($method, DTOMessage $request, $resultClass) {
		...
		if (defined('__LOCAL_DEBUG__') && !defined('SIMPLE_TEST')) {
			...
			Assert::isTrue(!$form->getErrors() && $request->entityProto()->validate($request, $form),
		...
		if (!$resultClass) {
			...
			Assert::isTrue($result->entityProto()->validate($result, $form),
