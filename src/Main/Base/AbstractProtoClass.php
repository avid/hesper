...
abstract class AbstractProtoClass extends Singleton {
			...
			foreach ($objectList as $object) {
				...
				if ($innerList[$i]) {
					try {
						...
						$object->$setter($innerList[$i]->dao()
						                               ->getById($innerList[$i]->getId()));
	...
	public function importPrimitive($path, Form $form, BasePrimitive $prm, /* Prototyped */
	                                $object, $ignoreNull = true) {
	...
	public function exportPrimitive($path, BasePrimitive $prm, /* Prototyped */
	                                $object, $ignoreNull = true) {
		if (strpos($path, ':') !== false) {
			...
			if ($property->getRelationId() == MetaRelation::ONE_TO_ONE && ($property->getFetchStrategyId() != FetchStrategy::LAZY)) {
				...
				if (($inner = $object->$getter()) instanceof DAOConnected) {
					if ($proto->depth) {
						...
						$object->$setter($inner->dao()
						                       ->getById($inner->getId()));
					}
				} elseif ($proto->depth // emulating 'instanceof DAOConnected'
	...
	private function forwardPrimitive($path, Form $form = null, BasePrimitive $prm, /* Prototyped */
	                                  $object, $ignoreNull = true) {
		...
		if ($form) {
			return $property->getProto()
			                ->importPrimitive($path, $form, $prm, $object->$getter(), $ignoreNull);
		} else {
			return $property->getProto()
			                ->exportPrimitive($path, $prm, $object->$getter(), $ignoreNull);
		}
	}
