...
use Hesper\Main\Util\IO\InputStream;
...
final class HtmlTokenizer {
	...
	private function getNextChar() {
		...
		if ($this->char == "\n" && $this->previousChar != "\r" || $this->char == "\r") {
			++$this->line;
			$this->linePosition = 1;
		} else {
			++$this->linePosition;
	...
	private function mark() {
		$this->mark = [$this->char, $this->previousChar, $this->line, $this->linePosition];
	...
	private function checkSpecialTagState() {
		...
		$specialStartTags = ['![CDATA[' => self::CDATA_STATE, '!--' => self::COMMENT_STATE];
	...
	private function attrValueState() {
		...
		while ($this->char !== null) {
			...
			if (!$this->insideQuote && self::isSpacerChar($this->char)) {
				...
				if ($this->char == '"' || $this->char == "'" || $this->char == $this->insideQuote // may be '>'
				) {
					if (!$this->insideQuote) {
