...
abstract class ProtoDAO extends GenericDAO {
	...
	public function fetchCollections(array $collections, array $list) {
		...
		foreach ($collections as $path => $info) {
			...
			if ($criteria = $info['criteria']) {
				$query = $criteria->setDao($this)
				                  ->fillSelectQuery($query);
			}
			...
			if ($lazy) {
				...
				$field = DBField::create($childId, $self->$getter()
				                                        ->getHelperTable());
	...
	protected function setQueryFields(InsertOrUpdateQuery $query, $object) {
		...
		return $this->getProtoClass()
		            ->fillQuery($query, $object);
	}
	...
	private function processPath(AbstractProtoClass $proto, $probablyPath, JoinCapableQuery $query, $table, $parentRequired = true, $prefix = null) {
		...
		if ($property->getRelationId() == MetaRelation::ONE_TO_MANY || $property->getRelationId() == MetaRelation::MANY_TO_MANY) {
			...
			if ($property->getRelationId() == MetaRelation::MANY_TO_MANY) {
				$helperTable = $self->$getter()
				                    ->getHelperTable();
				$helperAlias = $helperTable;

				if (!$query->hasJoinedTable($helperAlias)) {
					$logic = Expression::eq(DBField::create($this->getIdName(), $table),

						DBField::create($self->$getter()
				...
				$logic = Expression::eq(DBField::create($propertyDao->getIdName(), $alias),

					DBField::create($self->$getter()
					                     ->getChildIdField(), $helperAlias));
			} else {
				$logic = Expression::eq(DBField::create($self->$getter()
				                                             ->getParentIdField(), $alias),

					DBField::create($this->getIdName(), $table));
			}
			...
			if (!$query->hasJoinedTable($alias)) {
				$logic = Expression::eq(DBField::create($property->getColumnName(), $table),

					DBField::create($propertyDao->getIdName(), $alias));
	...
	public function guessAtom($atom, JoinCapableQuery $query, $table = null, $parentRequired = true, $prefix = null) {
		...
		if (is_string($atom)) {
			if (strpos($atom, '.') !== false) {
				return $this->processPath(call_user_func([$this->getObjectName(), 'proto']), $atom, $query, $table, $parentRequired, $prefix);
