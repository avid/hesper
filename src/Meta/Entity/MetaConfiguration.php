...
use Hesper\Meta\Builder\ContainerClassBuilder;
...
final class MetaConfiguration extends Singleton implements Instantiatable {
	...
	public function load($metafile, $generate = true) {
		...
		foreach ($this->liaisons as $class => $parent) {
			if (isset($this->classes[$parent])) {
				...
				Assert::isFalse($this->classes[$parent]->getTypeId() == MetaClassType::CLASS_FINAL,
	...
	public function buildSchemaChanges() {
		...
		foreach ($this->classes as $class) {
			...
			try {
				...
				$out->errorLine(get_class($db) . ' does not support tables introspection yet.',
			...
			if (
				...
				foreach ($class->getInterfaces() as $interface) {
					Assert::isTrue($info->implementsInterface($interface),
						...
						'class ' . $class->getName() . ' expected to implement interface ' . $interface);
				}
				...
				foreach ($class->getProperties() as $name => $property) {
					Assert::isTrue($property->toLightProperty($class) == $proto->getPropertyByName($name),

						'defined property does not match autogenerated one - ' . $class->getName() . '::' . $property->getName());
				}
	...
	private function makeProperty($name, $type, MetaClass $class, $size) {
		...
		$typeClass = '\Hesper\Meta\Type\\' . $type . 'Type';
		if (!class_exists($typeClass)) {
			$typeClass = '\Hesper\Meta\Type\ObjectType';
		}
	...
	private function processClasses(\SimpleXMLElement $xml, $metafile, $generate) {
		...
		foreach ($xml->classes[0] as $xmlClass) {
			...
			if (isset($xmlClass->properties[0]->identifier)) {
				...
				$property = $this->makeProperty($name, $type, $class, // not casting to int because of Numeric possible size
					(string)$id['size']);
	...
	private function loadXml($metafile, $generate) {
		...
		$contents = str_replace('"meta.dtd"', '"' . HESPER_META . 'meta.dtd"', $contents);
