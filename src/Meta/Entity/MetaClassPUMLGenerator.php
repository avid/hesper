...
final class MetaClassPUMLGenerator extends StaticFactory {
	...
	public static function generateLinks(array $classes) {
		...
		foreach ($classes as $class) {
			...
			foreach ($class->getProperties() as $property) {
				if ($property->getType() instanceof ObjectType && $property->getType()
				                                                           ->getClassName() && $property->getRelation()
				) {
					...
					$links[] = $class->getName() . $rel . $property->getType()
					                                               ->getClassName() . "\n";
